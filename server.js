...
app.post('/api/auth/register', async (req, res) => {
    ...
    db.users.push(user);
    writeDB(db);
    res.json({ success: true, token: jwt.sign({ id: user.id, role: user.role }, 'secret'), user });
});
...
app.post('/api/auth/login', async (req, res) => {
    ...
    res.json({ success: true, token: jwt.sign({ id: user.id, role: user.role }, 'secret'), name: user.name });
});
...
app.get('/api/stats', (req, res) => { const db = readDB(); res.json({ success: true, stats: { totalProducts: db.products.length, totalDoctors: db.doctors.length, totalOrders: (db.orders || []).length, totalAppointments: (db.appointments || []).length, lowStock: db.products.filter(p => p.totalStock < 10).length } }); });
