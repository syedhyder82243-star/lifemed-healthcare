...
app.put('/api/auth/profile', auth, async (req, res) => {
    ...
    if (index !== -1) {
        ...
        if (req.body.password) db.users[index].password = await bcrypt.hash(req.body.password, 10);
        writeDB(db);
        res.json({ success: true });
    } else {
        res.status(404).json({ success: false });
...
app.post('/api/admin/workers', auth, adminAuth, async (req, res) => {
    ...
    const hashed = await bcrypt.hash(password, 10);
    db.users.push({ id: Date.now(), name, email, password: hashed, role: role || 'staff' });
    writeDB(db);
    res.json({ success: true });
});
